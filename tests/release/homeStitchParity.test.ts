...
describe("home stitch parity", () => {
  ...
  it("uses a date-stable daily prompt for the primary diary card", () => {
    ...
    expect(homePage).toContain("resolveHomeHeroTitle");
    expect(homePage).toContain("settingsStore.homeTitleMode");
    expect(homePage).toContain("settingsStore.homeCustomTitle");
    expect(homePage).not.toContain("settingsStore.homeTitle?.trim() || copy.home.title");
