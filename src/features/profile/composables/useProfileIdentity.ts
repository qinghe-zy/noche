...
function sanitizeEditableText(value: string | null | undefined): string {
  return value?.trim() ?? "";
}
...
export function useProfileIdentity() {
  ...
  async function setDisplayName(displayName: string): Promise<void> {
    const nextValue = sanitizeEditableText(displayName);
  ...
  async function setSignature(signature: string): Promise<void> {
    const nextValue = sanitizeEditableText(signature);
    await persistIdentityField(
  ...
  async function setCoverUri(coverUri: string | null): Promise<void> {
    const nextValue = sanitizeUri(coverUri) ?? "";
    await persistIdentityField(
      PROFILE_PREF_KEYS.coverUri,
      nextValue,
      () => {
        identity.value = {
          ...identity.value,
          coverUri: sanitizeUri(nextValue),
        };
      },
      "保存背景图失败。",
    );
