...
export const PROFILE_PREVIEW_LIMIT = 4;
export const PROFILE_APP_VERSION = APP_VERSION;
export const PROFILE_DISPLAY_NAME_MAX = 10;
export const PROFILE_HOME_TITLE_MAX = 8;
...
export function countDisplayNameCharacters(displayName: string): number {
  return Array.from(displayName.trim()).length;
}

export function isDisplayNameWithinLimit(displayName: string): boolean {
  return countDisplayNameCharacters(displayName) <= PROFILE_DISPLAY_NAME_MAX;
}

export function isHomeTitleWithinLimit(title: string): boolean {
  return countDisplayNameCharacters(title) <= PROFILE_HOME_TITLE_MAX;
}
